...
def run_scipy_dea(
    ...
    try:
        ...
        # 계산 결과 로그 출력
        logger.info("DEA calculation completed, checking result stats...")
        
        # 기본적인 랭킹 결과 통계 표시
        if 'fee' in df_result.columns:
            top_by_score = df_result.sort_values('dea_score', ascending=False).head(10)
            logger.info(f"Top 10 plans by DEA score:\n{top_by_score[['dea_score', 'fee']].to_string()}")
            
            # 가격 기준 정렬
            top_by_price = df_result.sort_values('fee').head(10)
            logger.info(f"Top 10 plans by lowest price:\n{top_by_price[['dea_score', 'fee']].to_string()}")
            
        # 핵심 스펙 정보가 있으면 추가 로깅
        if 'basic_data_clean' in df_result.columns:
            # 데이터 용량 기준 정렬
            top_by_data = df_result.sort_values('basic_data_clean', ascending=False).head(10)
            logger.info(f"Top 10 plans by data amount:\n{top_by_data[['dea_score', 'fee', 'basic_data_clean']].to_string()}")
            
        # 간단한 통계 정보 출력
        logger.info(f"DEA scores - min: {df_result['dea_score'].min():.2f}, max: {df_result['dea_score'].max():.2f}, mean: {df_result['dea_score'].mean():.2f}")
        logger.info(f"Total plans processed: {len(df_result)}")
        ...
        # Log the top plans by rank
        top_plans = df_result.sort_values('dea_rank').head(10)
